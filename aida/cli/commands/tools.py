...
def init():
    ...
    async def _init():
        ...
        try:
            ...
            console.print(
                f"[success]✅ Successfully initialized {len(tool_names)} tools:[/success]"
            )
            for tool_name in sorted(tool_names):
...
[bold]Dependencies:[/bold] {', '.join(capability.dependencies) if capability.dependencies else 'None'}"""
        ...
        if capability.parameters:
            ...
            for param in capability.parameters:
                ...
                param_table.add_row(
                    param.name, param.type, required_text, default_text, param.description
                )
...
def execute(
    ...
    params: str | None = typer.Option(
        None, "--params", "-p", help="JSON parameters for tool execution"
    ),
    interactive: bool = typer.Option(
        False, "--interactive", "-i", help="Interactive parameter input"
    ),
    output_format: str = typer.Option(
        "pretty", "--format", "-f", help="Output format (pretty, json, raw)"
    ),
):
    ...
    async def _execute():
        ...
        if interactive:
            ...
            for param in capability.parameters:
                if param.required or console.confirm(
                    f"Set optional parameter '{param.name}'?", default=False
                ):
...
[bold]Duration:[/bold] {result.duration_seconds:.2f}s"""
                ...
                console.print_panel(
                    result_content, title="Execution Result", border_style=status_color
                )
...
def stats(
    tool_name: str | None = typer.Argument(
        None, help="Tool name (if not provided, shows all tools)"
    ),
    detailed: bool = typer.Option(False, "--detailed", "-d", help="Show detailed statistics"),
):
    """Show tool usage statistics."""

    async def _stats(tool_name=tool_name, detailed=detailed):
        registry = get_tool_registry()
        ...
        if tool_name:
            ...
            for tool_name, stats in all_stats.items():
                ...
                summary_table.add_row(
                    tool_name, str(total), f"{success_rate:.1f}%", f"{avg_duration:.2f}s"
                )
...
def capabilities(
    tool_name: str | None = typer.Argument(
        None, help="Tool name (if not provided, shows all capabilities)"
    ),
    output_format: str = typer.Option(
            ...
            if output_format == "json":
                ...
                for cap in sorted(capabilities, key=lambda x: x.name):
                    cap_table.add_row(
                        ...
                        ", ".join(cap.supported_platforms[:2])
                        + ("..." if len(cap.supported_platforms) > 2 else ""),
                    )
            ...
            for param in capability.parameters:
                if not param.name or not param.type:
                    ...
                    validation_results["issues"].append(
                        f"Invalid parameter definition: {param.name}"
                    )
        except Exception as e:
            validation_results["parameter_validation"] = False
