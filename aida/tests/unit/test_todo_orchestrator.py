...
from aida.core.orchestrator import (
    TodoOrchestrator, TodoPlan, TodoStep, TodoStatus, ReplanReason,
    get_todo_orchestrator
)
from aida.tools.base import ToolResult, ToolCapability, ParameterSpec
...
def mock_tool_registry():
    ...
    thinking_capability = ToolCapability(
        ...
        parameters=[
            ParameterSpec(name="problem", type="string", description="Problem to analyze", required=True),
            ParameterSpec(name="reasoning_type", type="string", description="Type of reasoning", required=False)
        ]
    )
    thinking_tool.get_capability.return_value = thinking_capability
    thinking_tool.execute_async = AsyncMock(return_value=ToolResult(
        status="success",
    ...
    execution_capability = ToolCapability(
        ...
        parameters=[
            ...
            ParameterSpec(name="language", type="string", description="Programming language", required=False)
        ]
    )
    execution_tool.get_capability.return_value = execution_capability
    execution_tool.execute_async = AsyncMock(return_value=ToolResult(
        status="success",
        result={"output": "Hello World", "exit_code": 0},
        duration_seconds=0.5
    ))
    
    registry.get_tool = AsyncMock(side_effect=lambda name: {
...
class TestTodoPlan:
    ...
    def test_todo_plan_creation(self):
        ...
        steps = [
            ...
            TodoStep(id="step_002", description="Second step", tool_name="execution", parameters={})
        ]
    ...
    def test_next_executable_step(self):
        ...
        steps = [
            TodoStep(id="step_001", description="Step 1", tool_name="thinking", parameters={}),
            TodoStep(id="step_002", description="Step 2", tool_name="execution", 
                    parameters={}, dependencies=["step_001"]),
            TodoStep(id="step_003", description="Step 3", tool_name="thinking", parameters={})
        ]
    ...
    def test_should_replan(self):
        ...
        # Add failed step
        failed_step = TodoStep(id="step_001", description="Failed step", 
                              tool_name="thinking", parameters={})
    ...
    def test_markdown_generation(self):
        ...
        steps = [
            TodoStep(id="step_001", description="Analyze problem", tool_name="thinking", parameters={}),
            TodoStep(id="step_002", description="Execute solution", tool_name="execution", parameters={})
        ]
        steps[0].status = TodoStatus.COMPLETED
...
```'''
    ...
    async def test_execute_plan_success(self, orchestrator, mock_llm_manager):
        ...
        steps = [
            TodoStep(id="step_001", description="Think", tool_name="thinking", 
