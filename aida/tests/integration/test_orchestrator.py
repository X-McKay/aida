...
class LoggingOrchestrator(TodoOrchestrator):
    ...
    async def create_plan(self, user_request: str, context=None):
        ...
        try:
            ...
            self.llm_interactions.append(
                {"user_request": user_request, "success": False, "error": str(e)}
            )
            raise
...
class OrchestratorTestSuite(BaseTestSuite):
    ...
    async def test_basic_plan_creation(self) -> dict[str, Any]:
        ...
        try:
            ...
            if hasattr(orchestrator, "storage_manager") and hasattr(
                orchestrator.storage_manager, "storage_dir"
            ):
            ...
            return {"success": False, "message": f"Plan generation error: {str(e)}"}
        except Exception as e:
            # Handle other unexpected errors
            return {"success": False, "message": f"Unexpected error during plan creation: {str(e)}"}
    ...
    async def test_complex_plan_creation(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not plan or not plan.steps:
                return {
                    "success": False,
                    "message": "Failed to create complex plan - no plan or steps",
                }
    ...
    async def test_plan_progress_tracking(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "message": f"Progress tracking failed: {str(e)}"}
        ...
        if failed:
            ...
            for i, interaction in enumerate(failed):
                ...
                response_preview = (
                    ...
                    if len(interaction["response"]) > 100
