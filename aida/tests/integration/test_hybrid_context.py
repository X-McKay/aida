...
This additional content helps test compression and summarization features."""
    ...
    async def test_core_interface_compress(self) -> dict[str, Any]:
        ...
        if compression_stats.get("compressed_size", 0) < compression_stats.get("original_size", 1):
            ...
            return {"success": False, "message": "Compression did not reduce size"}
    ...
    async def test_core_interface_summarize(self) -> dict[str, Any]:
        ...
        if isinstance(summary, dict) and "overview" in summary:
            ...
            return {"success": False, "message": "Summary not properly structured"}
    ...
    async def test_pydantic_ai_interface(self) -> dict[str, Any]:
        ...
        try:
            result = await tools["compress_context"](
                content=self.test_content, compression_level="aggressive", priority="recency"
            )
            ...
            if "compressed_content" in result and "compression_stats" in result:
                ...
                return {"success": False, "message": "Unexpected result structure"}

        except Exception as e:
            return {"success": False, "message": f"PydanticAI interface error: {str(e)}"}
    ...
    async def test_mcp_server_interface(self) -> dict[str, Any]:
        ...
        if "context_compress" not in tool_names:
            return {"success": False, "message": "MCP tools not properly registered"}

        # Test summarization via MCP
        result = await mcp_server.call_tool(
            "context_summarize",
            {"content": self.test_content, "max_tokens": 300, "format_type": "bullet_points"},
        )

        if result.get("isError", False):
            error_text = result.get("content", [{}])[0].get("text", "Unknown error")
            return {"success": False, "message": f"MCP execution error: {error_text}"}

        # Parse the JSON response from content
        import json

        response_text = (
            result.get("content", [{}])[0].get("text", "{}") if isinstance(result, dict) else "{}"
        )
        try:
        ...
        if "summary" in response_data:
            ...
            return {"success": False, "message": "Unexpected MCP result"}
    ...
    async def test_observability_interface(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not hasattr(observability, "tracer"):
                return {"success": False, "message": "Tracer not initialized"}

            if not hasattr(observability, "compression_ratio"):
                return {"success": False, "message": "Metrics not initialized"}
            ...
            return {"success": False, "message": f"Observability error: {str(e)}"}
    ...
    async def test_key_point_extraction(self) -> dict[str, Any]:
        ...
        result = await self.tool.execute(
            operation="extract_key_points", content=self.test_content, max_results=5
        )
        ...
        if len(key_points) > 0 and all("category" in kp for kp in key_points):
            ...
            return {"success": False, "message": "No key points extracted"}
    ...
    async def test_token_optimization(self) -> dict[str, Any]:
        ...
        result = await self.tool.execute(
            operation="optimize_tokens", content=self.test_content, max_tokens=100
        )
        ...
        if token_analysis.get("final_tokens", 0) <= token_analysis.get("target_tokens", 100):
            ...
            return {"success": False, "message": "Token optimization did not meet target"}
    ...
    async def test_context_search(self) -> dict[str, Any]:
        ...
        result = await self.tool.execute(
            operation="search", content=self.test_content, query="hybrid patterns"
        )
    ...
    async def test_snapshot_operations(self) -> dict[str, Any]:
        ...
        try:
            ...
            create_result = await self.tool.execute(
                operation="snapshot", content=json.dumps(context_data), file_path=snapshot_path
            )

            if create_result.status != ToolStatus.COMPLETED:
                return {
                    "success": False,
                    "message": f"Snapshot creation failed: {create_result.error}",
                }

            # Restore snapshot
            restore_result = await self.tool.execute(operation="restore", file_path=snapshot_path)

            if restore_result.status != ToolStatus.COMPLETED:
                return {
                    "success": False,
                    "message": f"Snapshot restore failed: {restore_result.error}",
                }
