    ...
    async def test_aida_interface_file_operations(self) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self.tool.execute(
                operation="read", path=str(self.test_files["simple.txt"])
            )
    ...
    async def test_pydantic_ai_compatibility(self) -> dict[str, Any]:
        ...
        try:
            ...
            write_result = await pydantic_tools["write_file"](
                str(test_file), json.dumps(new_data, indent=2)
            )
    ...
    async def test_mcp_server_integration(self) -> dict[str, Any]:
        ...
        try:
            ...
            write_result = await mcp_server.call_tool(
                "file_operations_write", {"path": str(mcp_test_file), "content": test_content}
            )
            ...
            read_result = await mcp_server.call_tool(
                "file_operations_read", {"path": str(mcp_test_file)}
            )
            ...
            list_result = await mcp_server.call_tool(
                "file_operations_list", {"path": self.temp_dir}
            )
            ...
            return {
                "success": False,
                "message": f"MCP server test failed: {e}",
                "traceback": traceback.format_exc(),
            }
    ...
    async def test_observability_integration(self) -> dict[str, Any]:
        ...
        try:
            ...
            observability = self.tool.enable_observability(
                {"enabled": True, "service_name": "test-hybrid-file-ops"}
            )
            ...
            with observability.trace_operation("test_trace", path=str(test_file)):
                ...
                result = await self.tool.execute(
                    operation="write", path=str(test_file), content="Traced operation content"
                )
    ...
    async def test_hybrid_architecture_compatibility(self) -> dict[str, Any]:
        ...
        try:
            ...
            aida_result = await self.tool.execute(
                operation="write", path=str(test_file), content=aida_content
            )
            ...
            mcp_result = await mcp_server.call_tool(
                "file_operations_write", {"path": str(test_file), "content": mcp_content}
            )
            assert not mcp_result.get("isError")

            # 4. Verify with AIDA interface
            verify_result = await self.tool.execute(operation="read", path=str(test_file))
        ...
        try:
            ...
            for _ in range(10):
                await self.tool.execute(
                    operation="write", path=str(test_file), content=test_content
                )
