...
class HybridExecutionTestSuite(BaseTestSuite):
    ...
    async def test_core_interface_python(self) -> dict[str, Any]:
        ...
        stdout = (
            result.result.get("output", result.result.get("stdout", ""))
        ...
        if expected_output in stdout:
            ...
            return {"success": False, "message": f"Unexpected output: {stdout[:100]}"}
    ...
    async def test_core_interface_bash(self) -> dict[str, Any]:
        ...
        if "Hello from Bash!" in stdout:
            return {"success": True, "message": "Bash execution successful", "output": stdout[:100]}
        else:
            return {"success": False, "message": f"Unexpected output: {stdout[:100]}"}
    ...
    async def test_mcp_server_interface(self) -> dict[str, Any]:
        ...
        if "MCP execution test" in stdout:
            return {"success": True, "message": "MCP interface working", "output": stdout[:100]}
        else:
            return {"success": False, "message": f"Unexpected output: {stdout[:100]}"}
...
""",
        ...
        if "Hello from file!" in stdout:
            return {"success": True, "message": "File execution successful", "output": stdout[:100]}
        else:
            return {"success": False, "message": f"Unexpected output: {stdout[:100]}"}
...
""",
        ...
        if (
            ...
            and result.error
            and ("timeout" in result.error.lower() or "timed out" in result.error.lower())
        ):
            return {
                "success": True,
