...
class ChatTestRunner:
    ...
    async def run_question(self, question: str, expected_tools: list[str] = None) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self.orchestrator.execute_request(question, context={"test_mode": True})
    ...
    async def run_category_tests(
        self, category: str, questions: list[str], expected_tool_count: int = None
    ) -> dict[str, Any]:
            ...
            if result["success"]:
                ...
                if expected_tool_count is not None:
                    ...
                    if actual_count == expected_tool_count:
                        if self.verbose:
                            print(
                                f"    ✅ Success (used {actual_count} tools: {', '.join(result['tools_used'])})"
                            )
                    else:
                        if self.verbose:
                            print(
                                f"    ⚠️  Tool count mismatch: expected {expected_tool_count}, got {actual_count}"
                            )
                else:
                    if self.verbose:
                        print(
                            f"    ✅ Success (used: {', '.join(result['tools_used']) or 'no tools'})"
                        )
            else:
                if self.verbose:
    ...
    async def run_all_tests(self) -> dict[str, Any]:
        ...
        test_categories = [
            ...
            (
                "Multi Tool Required",
                test_suite.multi_tool_questions[:5],
                None,
            ),  # Don't enforce count
        ]
        ...
        for category_name, questions, expected_tools in test_categories:
            category_result = await self.run_category_tests(
                category_name, questions, expected_tools
            )
        ...
        for cat_result in all_results:
            print(
                f"  • {cat_result['category']}: {cat_result['successful']}/{cat_result['total_questions']} "
