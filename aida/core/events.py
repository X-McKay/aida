...
class EventBus:
    ...
    def subscribe(
        ...
        subscription = EventSubscription(
            handler=handler, event_filter=event_filter, subscription_id=subscription_id
        )
    ...
    async def wait_for_event(
        self, event_type: str, timeout: float = 30.0, event_filter: EventFilter | None = None
    ) -> Event | None:
        ...
        subscription_id = self.subscribe(event_type, handler, event_filter)
    ...
    def _find_matching_subscriptions(self, event: Event) -> list[EventSubscription]:
        ...
        for sub_id, subscription in self._subscriptions.items():
            if (
                subscription.filter is None
                or subscription.filter.event_types is None
                or sub_id in subscription_ids
...
def subscribe_to_events(
    event_types: str | list[str], handler: EventHandler, event_filter: EventFilter | None = None
) -> str:
