...
class TodoStep(BaseModel):
    ...
    def to_markdown_line(self) -> str:
        ...
        checkbox = "[x]" if self.status == TodoStatus.COMPLETED else "[!]" if self.status == TodoStatus.FAILED else "[~]" if self.status == TodoStatus.IN_PROGRESS else "[-]" if self.status == TodoStatus.SKIPPED else "[ ]"
    ...
    def get_status_display(self) -> str:
        ...
        if self.status == TodoStatus.COMPLETED:
            ...
            retry_str = f" (Retry {self.retry_count}/{self.max_retries})" if self.retry_count > 0 else ""
...
class TodoPlan(BaseModel):
    ...
    def should_replan(self) -> tuple[bool, Optional[ReplanReason]]:
        ...
        if completed_since_last >= OrchestratorConfig.REPLAN_AFTER_STEPS or time_since_evaluation > OrchestratorConfig.REPLAN_AFTER_SECONDS:
    ...
    def to_terminal_display(self) -> str:
        ...
        for i, step in enumerate(self.steps, 1):
            ...
            if step.status == TodoStatus.FAILED and step.error:
                task_line += f" (Error: {step.error[:30]}...)" if len(step.error) > 30 else f" (Error: {step.error})"
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        if self.steps:
            ...
            last_complete = max((s.completed_at for s in self.steps if s.completed_at), default=None)
        ...
        failed_steps = [
            ...
            for step in self.steps if step.status == TodoStatus.FAILED
        ]
