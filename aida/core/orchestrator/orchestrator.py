...
class TodoOrchestrator:
    ...
    def __init__(self, storage_dir: str = None):
        ...
        self.storage_manager = PlanStorageManager(storage_dir or OrchestratorConfig.DEFAULT_STORAGE_DIR)
    ...
    def _validate_plan_data(self, plan_data: Dict[str, Any]) -> None:
        ...
        if len(plan_data["execution_plan"]) > OrchestratorConfig.MAX_STEPS_PER_PLAN:
            raise ValueError(f"Plan contains too many steps (max: {OrchestratorConfig.MAX_STEPS_PER_PLAN})")
    ...
    def load_plan(self, plan_id: str) -> Optional[TodoPlan]:
    ...
    def display_plan(self, plan_id: str) -> Optional[str]:
    ...
    async def _generate_initial_plan(self, user_request: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Raw LLM response causing error: {response[:1000] if 'response' in locals() else 'No response'}")
            # Don't return a fallback plan - raise a clear error instead
            raise RuntimeError(f"Plan generation failed: {e}. The LLM did not return a valid JSON response. Please check your LLM configuration and try again.")
    ...
    def _extract_json_from_response(self, response: str) -> str:
        ...
        raise ValueError(f"No valid JSON found in response. Response length: {len(response_text)}, preview: {response_text[:200]}...")
