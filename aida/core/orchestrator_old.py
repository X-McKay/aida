...
from aida.config.llm_defaults import get_reasoning_system_prompt, get_tool_orchestration_prompt
from aida.providers.llm.base import LLMMessage, LLMRole
from aida.providers.llm.manager import get_llm_manager
from aida.tools.base import ToolResult, get_tool_registry
...
class WorkflowStep:
    ...
    def _get_result_summary(self) -> dict[str, Any] | None:
        if not self.result:
            return None
...
class WorkflowPlan:
    ...
    def __init__(
        self, user_request: str, analysis: str, steps: list[WorkflowStep], expected_outcome: str
    ):
...
class WorkflowOrchestrator:
    ...
    async def process_request(
        ...
        self.conversation_history.append(LLMMessage(role=LLMRole.USER, content=user_message))
    ...
    async def execute_workflow(
        self, workflow: WorkflowPlan, progress_callback: callable | None = None
    ) -> dict[str, Any]:
    ...
    async def _execute_step(self, step: WorkflowStep, retry_count: int = 0) -> dict[str, Any]:
        ...
        try:
            ...
            if step.tool_name == "execution" and result and hasattr(result, "result"):
                ...
                if isinstance(execution_result, dict):
                    ...
                    if exit_code != 0 and retry_count < 2:
                        logger.warning(
                            f"🔄 Execution failed (exit {exit_code}), attempting auto-correction..."
                        )

                        # Attempt to fix the error
                        corrected_step = await self._attempt_error_correction(
                            step, execution_result, retry_count
                        )
                        if corrected_step:
                            logger.info(
                                f"🛠️ Retrying with corrected code (attempt {retry_count + 1}/2)..."
                            )
            ...
            return {"step": step.to_dict(), "success": False, "error": str(e)}

    async def _attempt_error_correction(
        self, failed_step: WorkflowStep, execution_result: dict[str, Any], retry_count: int
    ) -> WorkflowStep | None:
        ...
        try:
            ...
            if hasattr(self, "llm_manager") and self.llm_manager:
                try:
                    ...
                    correction_data = self._parse_error_correction_response(
                    ...
                    if correction_data and correction_data.get("corrected_code"):
                        ...
                        logger.info(
                            f"🔧 LLM-corrected code: {correction_data.get('changes_made', 'Fixed errors')}"
                        )
    ...
    def _parse_error_correction_response(self, response_content: str) -> dict[str, Any] | None:
    ...
    def _simple_error_correction(
        self, failed_step: WorkflowStep, execution_result: dict[str, Any]
    ) -> WorkflowStep | None:
        ...
        try:
            ...
            for i, step in enumerate(parsed_response.get("execution_plan", [])):
                ...
                if tool_name not in valid_tools:
                    logger.warning(
                        f"Step {i+1}: unknown tool '{tool_name}', valid tools: {valid_tools}"
                    )
            ...
            raise ValueError(
                f"LLM response parsing failed: {e}. Raw response logged for debugging."
            )
