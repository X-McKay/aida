...
class InMemoryStore(MemoryStore):
    ...
    async def _evict_oldest(self) -> None:
        ...
        entries_by_score = sorted(
            self._entries.items(), key=lambda x: (x[1].access_count, x[1].accessed_at)
        )
...
class MemoryManager:
    ...
    async def retrieve_memory(self, memory_id: str) -> MemoryEntry | None:
    ...
    async def compact_conversation(self, context_id: str, compression_ratio: float = 0.5) -> bool:
    ...
    async def cleanup(self) -> dict[str, int]:
        ...
        return {"expired_memories": expired_memories, "expired_contexts": expired_contexts}
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        return {**self._stats, "store": store_stats, "contexts": context_stats}
