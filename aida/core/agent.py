...
class BaseAgent(ABC):
    ...
    @abstractmethod
    async def process_message(self, message: Any) -> Any | None:
    ...
    async def send_message(
        self, protocol_name: str, message: Any, recipient: str | None = None
    ) -> bool:
    ...
    def _setup_protocols(self) -> None:
        ...
        if "a2a" in self.config.protocols:
            ...
            self.protocols["a2a"] = A2AProtocol(agent_id=self.agent_id, **a2a_config)

        # Setup MCP protocol
        if "mcp" in self.config.protocols:
            mcp_config = self.config.protocols["mcp"]
            self.protocols["mcp"] = MCPProtocol(agent_id=self.agent_id, **mcp_config)
    ...
    async def _message_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                for _protocol_name, protocol in self.protocols.items():
