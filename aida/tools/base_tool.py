...
class BaseModularTool(ABC, Generic[TRequest, TResponse, TConfig]):
    ...
    def enable_observability(self, config: dict[str, Any] | None = None) -> Any:
        """Enable OpenTelemetry observability."""
        if self._observability is None:
            obs_config = config or {}
            obs_config.setdefault(
                "service_name", f"{BaseToolConfig.SERVICE_NAME_PREFIX}-{self.name}"
            )
        ...
        try:
            ...
            if self._observability:
                ...
                self._observability.record_operation(
                    operation=operation, duration=duration, success=True
                )
            ...
            if self._observability:
                duration = (datetime.utcnow() - start_time).total_seconds()
                self._observability.record_operation(
                    operation=kwargs.get("operation", "unknown"), duration=duration, success=False
                )
