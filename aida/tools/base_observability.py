...
class BaseObservability:
    ...
    def record_operation(self, operation: str, duration: float, success: bool, **custom_metrics):
...
class SimpleObservability(BaseObservability):
    ...
    def __init__(self, tool, config: dict[str, Any], custom_metrics: dict[str, Any] | None = None):
        ...
        for metric_name, definition in self.custom_metric_definitions.items():
            ...
            if metric_type == "counter":
                self.custom_metrics[metric_name] = self.meter.create_counter(
                    full_name, description=description, unit=unit
                )
