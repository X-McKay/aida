...
class ContextTool(Tool):
    ...
    def get_capability(self) -> ToolCapability:
        ...
        return ToolCapability(
            ...
            parameters=[
                ...
                ToolParameter(
                    name="content", type="str", description="The content to process", required=True
                ),
                ToolParameter(
        ...
        try:
            ...
            if "compression_level" in kwargs:
                ...
                if isinstance(level, str):
                    ...
                    kwargs["compression_level"] = level_map.get(
                        level.lower(), CompressionLevel.MODERATE
                    )
    ...
    async def _search_context(self, request: ContextRequest) -> ContextResponse:
        ...
        search_results = await self.processor.search(
            content, request.query, request.max_results or ContextConfig.DEFAULT_MAX_SEARCH_RESULTS
        )
    ...
    async def _create_snapshot(self, request: ContextRequest) -> ContextResponse:
        ...
        # Create snapshot
        snapshot = await self.snapshot_manager.create_snapshot(content, request.file_path)
    ...
    async def _export_context(self, request: ContextRequest) -> ContextResponse:
        ...
        export_result = await self.processor.export_context(
            content, request.file_path, format_type.value
        )
    ...
    def _normalize_content(self, content: str | dict | list) -> str:
    ...
    def _create_tool_result(self, response: ContextResponse, start_time: datetime) -> ToolResult:
        """Create ToolResult from ContextResponse."""
        # Create a comprehensive result dict
        result = {"operation": response.operation.value, "status": response.status}
    ...
    async def _optimize_tokens(self, request: ContextRequest) -> ContextResponse:
        ...
        compressed = await self.processor.compress(
            content, compression_ratio, request.priority.value if request.priority else "balanced"
        )
    ...
    def enable_observability(self, config: dict[str, Any] | None = None):
        ...
        if self._observability is None:
            ...
            self._observability = ContextObservability(
                self, config or ContextConfig.get_observability_config()
            )
