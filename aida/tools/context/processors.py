...
class ContextProcessor:
    ...
    def _score_content_importance(self, content: str, priority: str) -> Dict[str, Any]:
        ...
        weights = ContextConfig.SCORING_WEIGHTS.get(priority, ContextConfig.SCORING_WEIGHTS["balanced"])
    ...
    def _calculate_keyword_score(self, text: str) -> float:
        ...
        importance_count = sum(
            1 for keyword in ContextConfig.IMPORTANCE_KEYWORDS
            if keyword in text_lower
        )
        
        action_count = sum(
            1 for keyword in ContextConfig.ACTION_KEYWORDS
            if keyword in text_lower
        )
        
        total_keywords = importance_count + action_count
        max_score = len(ContextConfig.IMPORTANCE_KEYWORDS) + len(ContextConfig.ACTION_KEYWORDS)
