...
from .models import (
    FileOperation, FileOperationRequest, FileOperationResponse,
    SearchScope, FileInfo
)
...
class FileOperationsTool(BaseModularTool[FileOperationRequest, FileOperationResponse, FilesConfig]):
    ...
    async def _write_file(self, request: FileOperationRequest) -> FileOperationResponse:
        ...
        return FileOperationResponse(
            ...
            path=str(path),
            result={
                "bytes_written": len(content.encode(request.encoding)),
                "path": str(path)
            },
            files_affected=1,
    ...
    async def _search_files(self, request: FileOperationRequest) -> FileOperationResponse:
        ...
        if path.is_file():
            ...
            files_to_search = [
                ...
                if f.is_file() and FilesConfig.is_text_file(str(f))
            ]
        elif path.is_dir():
            files_to_search = [
