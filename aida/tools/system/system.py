...
from .models import (
    SystemOperation, SystemRequest, SystemResponse,
    CommandResult, ProcessInfo, SystemInfo
)
...
class SystemTool(BaseModularTool[SystemRequest, SystemResponse, SystemConfig]):
        ...
        try:
            ...
            try:
                ...
                if len(result.stdout) > SystemConfig.MAX_OUTPUT_SIZE:
                    result.stdout = result.stdout[:SystemConfig.MAX_OUTPUT_SIZE] + SystemConfig.OUTPUT_TRUNCATE_MSG
                if len(result.stderr) > SystemConfig.MAX_OUTPUT_SIZE:
                    result.stderr = result.stderr[:SystemConfig.MAX_OUTPUT_SIZE] + SystemConfig.OUTPUT_TRUNCATE_MSG
    ...
    async def _list_processes(self, request: SystemRequest) -> SystemResponse:
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'status', 'cpu_percent', 'memory_percent',
                                             'create_time', 'username', 'cmdline', 'ppid']):
                try:
                    info = proc.info
    ...
    def _create_pydantic_tools(self) -> Dict[str, Callable]:
        ...
        async def which_command(command: str) -> Optional[str]:
            """Find command in PATH."""
            result = await self.execute(
                operation="which",
                command=command
            )
            return result.result if result.status == ToolStatus.COMPLETED and result.result else None
