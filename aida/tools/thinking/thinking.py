...
from .models import OutputFormat, Perspective, ReasoningType, ThinkingRequest, ThinkingResponse
...
class ThinkingTool(BaseModularTool[ThinkingRequest, ThinkingResponse, ThinkingConfig]):
    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            # Parse response
            response = self.response_parser.parse(llm_response, request)
    ...
    def _get_cache_key(self, request: ThinkingRequest) -> str:
        ...
        return (
            f"{request.problem[:50]}_{request.reasoning_type}_{request.depth}_{request.perspective}"
        )

    def _create_tool_result(self, response: ThinkingResponse, start_time: datetime) -> ToolResult:
        ...
        if response.sections:
            result_data["sections"] = {k: v.dict() for k, v in response.sections.items()}
    ...
    def _create_pydantic_tools(self) -> dict[str, Callable]:
        ...
        async def brainstorm_solutions(problem: str, context: str = "") -> dict[str, Any]:
            ...
            result = await self.execute(
                problem=problem, context=context, reasoning_type="brainstorming", depth=4
            )
            return result.result
        ...
        async def analyze_decision(
            decision: str, options: list[str], context: str = ""
        ) -> dict[str, Any]:
            ...
            result = await self.execute(
                problem=decision, context=full_context, reasoning_type="decision_analysis", depth=4
            )
            return result.result
