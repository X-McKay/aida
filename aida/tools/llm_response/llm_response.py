...
from .models import LLMResponseRequest
...
class LLMResponseTool(SimpleToolBase):
    ...
    def _get_tool_description(self) -> str:
        return (
            "Get a direct response from the LLM for questions, explanations, and general knowledge"
        )
    ...
    def _create_processors(self) -> dict[str, Callable]:
        return {"answer": self._answer_question}
    ...
    def _create_pydantic_tools(self) -> dict[str, Callable]:
        ...
        async def ask_llm(
            question: str, context: str = "", max_length: int = LLMResponseConfig.DEFAULT_MAX_LENGTH
        ) -> str:
            """Get a direct LLM response to a question."""
            result = await self.execute(question=question, context=context, max_length=max_length)
        ...
        async def explain_concept(concept: str, context: str = "") -> str:
            ...
            result = await self.execute(question=question, context=context, max_length=3000)
            return result.result

        async def answer_question(question: str, context: str = "") -> str:
            """Answer a question using the LLM."""
            result = await self.execute(question=question, context=context)
            return result.result
