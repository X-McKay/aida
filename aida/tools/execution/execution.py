...
from .models import ExecutionRequest, ExecutionResponse, ExecutionLanguage, ExecutionEnvironment
...
class ExecutionTool(BaseModularTool[ExecutionRequest, ExecutionResponse, ExecutionConfig]):
        ...
        async with dagger.Connection(dagger.Config(log_output=None)) as client:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                container = container.with_mounted_directory(
                    ExecutionConfig.CONTAINER_WORK_DIR,
                    client.host().directory(temp_dir)
                )
                ...
                try:
                    ...
                    result = await asyncio.wait_for(
                        container.with_exec(command).stdout(),
                        timeout=request.timeout
                    )
    ...
    def _create_pydantic_tools(self) -> Dict[str, Callable]:
        ...
        async def run_javascript(code: str, packages: Optional[List[str]] = None) -> str:
            """Run JavaScript code."""
            result = await self.execute(
                language="javascript",
                code=code,
